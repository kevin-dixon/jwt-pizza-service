...
class DB {
  ...
  async getUsers(page = 0, limit = 10, nameFilter = "*") {
    ...
    try {
      let users = await this.query(
        connection,
        `SELECT id, name, email FROM user WHERE name LIKE ? ORDER BY id LIMIT ${safeLimit + 1} OFFSET ${offset}`,
        [sqlFilter],
      );
  ...
  async getActiveUserCount() {
    ...
    try {
      const rows = await this.query(
        connection,
        `SELECT COUNT(DISTINCT userId) AS activeUserCount FROM auth`,
      );
      return Number(rows?.[0]?.activeUserCount || 0);
    } finally {
      connection.end();
    }
  }
  ...
  async query(connection, sql, params) {
    ...
    logger.log("info", "sql", sql, {
      params: JSON.stringify(logger.sanitize(params ?? [])),
    });
  ...
  async initializeDatabase() {
    try {
      ...
      try {
        ...
        const defaultAdminName = "常用名字";
        const defaultAdminEmail = "a@jwt.com";
        ...
        if (existingAdmin.length === 0) {
          ...
          const adminInsertResult = await this.query(
            connection,
            `INSERT INTO user (name, email, password) VALUES (?, ?, ?)`,
            [defaultAdminName, defaultAdminEmail, hashedPassword],
          );
        ...
        const adminRole = await this.query(
          connection,
          `SELECT role FROM userRole WHERE userId=? AND role=?`,
          [adminId, Role.Admin],
        );
